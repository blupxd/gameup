...
const UserProfile: React.FC<UserProfileProps> = ({ session }) => {
  ...
  if (!currentSession?.user?.username) {
    return (
      <>
        <div className="md:flex hidden items-center text-base space-x-8">
          <Link
            href="/login"
            className="px-10 hover:bg-[#4dc1bb] transition-colors duration-300 ease-in-out py-2 rounded text-[#1c1c1c] font-bold bg-[#5AECE5]"
          >
            Sign in
          </Link>
          <Link
            href="/register"
            className="px-10 py-2 hover:bg-[#23272c] transition-colors duration-300 ease-in-out rounded text-[#D9D9D9] font-bold bg-[#3A434D]"
          >
            Create an account
          </Link>
        </div>
